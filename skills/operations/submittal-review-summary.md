...
Use the **Reviewer-of-Platform-AI-Output sub-mode** when a platform AI tool has already processed the submittal and generated a discrepancy report, compliance check, or draft review memo. Platforms currently shipping at enterprise scale for AI-assisted submittal review include Trunk Tools TrunkSubmittal (enterprise rollout with top-10 GCs including Gilbane, Suffolk, and Cleveland Construction; vendor reports 35,000+ submittal reviews run and a median cycle-time reduction from ~55 days to ~14 days as of mid-2026 — vendor-reported, unaudited), Procore Datagrid AI (limited availability launched April 2026 — autonomous submittal reviews and RFI drafting), Pelles AI (submittal log generation and review), Newforma Vojo (agent-driven AI-assisted submittal review workflows announced at Newforma World 2026; integrated with Smart Email Filing), and others. The sub-mode treats the platform output as first-pass junior-reviewer work and applies the same 6-point compliance check this skill always applies, then produces a redlined version ready to stamp and route.
...
**A second, vendor-reported data point points the same direction.** One AI-submittal-review vendor operating at meaningful scale has disclosed (2026 mid-year, self-reported and unaudited — treat as directional, not a controlled study) that across tens of thousands of submittal reviews run through its platform, the large majority came back non-compliant or only partially compliant, with a small single-digit percentage fully compliant on first pass. Whatever the exact split at any given platform, the operational read-through is the same one this skill has always assumed: **a submittal reaching review should be treated as probably deficient until the compliance matrix says otherwise, not as probably fine.** That assumption is now backed by both an independent academic benchmark (above) and vendor-scale field data, from two unrelated sources.
