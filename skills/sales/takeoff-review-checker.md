...
- AI-generated takeoffs that ship with confidence scores or audit trails (Togal, Kreo, STACK 2026 with VisualSearch, Buildxact AI Estimate Reviewer, Beam AI — expanding from takeoff into full AI estimating + bidding after its 2026 Series B — CountBricks, XBuild chat-based estimating for roofing scopes, Rebar for HVAC / electrical / plumbing equipment counts, OpenConstructionERP for AGPL-3.0 open-source BOQ + AI cost-matching, Brickanta or ConCntric for agentic preconstruction OS platforms that unify estimating / planning / procurement and complete preconstruction tasks on the user's behalf). As of mid-2026 this is a venture-funded, multi-vendor category — and the agentic-preconstruction-OS sub-shape is now itself contested-multi-vendor (Brickanta and ConCntric — the latter's "Amplify" agentic assistant raised a 2026 Series A to drive estimating/planning/reporting), so the same discipline applies whether the number came from a single-purpose takeoff tool or an agentic platform that auto-completes the estimate. Treat any single platform's quantities and pricing as a first pass, never the controlling bid figure, and run the error-class sweep below regardless of the platform's reported confidence; an agentic OS that auto-advances a preconstruction estimate *raises* the value of an independent takeoff review, because the estimator — not the platform — owns the bid number.
