...
- AI-generated takeoffs that ship with confidence scores or audit trails (Togal, Kreo, STACK 2026 with VisualSearch, Buildxact AI Estimate Reviewer, Beam AI — expanding from takeoff into full AI estimating + bidding after its 2026 Series B — CountBricks, XBuild chat-based estimating for roofing scopes, Rebar for HVAC / electrical / plumbing equipment counts, Higharc — which after its 2026 Series C translates a floorplan image into a 3D spatial model and derives quantities from it for production residential — OpenConstructionERP for AGPL-3.0 open-source BOQ + AI cost-matching, Brickanta, ConCntric, or Provision for agentic preconstruction platforms that unify estimating / planning / procurement, surface document risks and opportunities, or complete preconstruction tasks on the user's behalf). As of mid-2026 this is a venture-funded, multi-vendor category — and the agentic-preconstruction sub-shape is now three vendors deep (Brickanta seed; ConCntric Series A, whose "Amplify" agentic assistant drives estimating/planning/reporting; and Provision's estimator co-pilot, seed-funded and already run across $100B+ of projects at large GCs, which reads the project documents to flag risks and opportunities for the estimator). The same discipline applies whether the number came from a single-purpose takeoff tool or an agentic platform that auto-completes the estimate or pre-reads the documents. Treat any single platform's quantities and pricing as a first pass, never the controlling bid figure, and run the error-class sweep below regardless of the platform's reported confidence; an agentic OS that auto-advances a preconstruction estimate *raises* the value of an independent takeoff review, because the estimator — not the platform — owns the bid number.
- **Takeoffs that feed procurement directly, not just the bid.** The 2026 funding wave is wiring AI quantities straight through to a purchasable material list — Higharc's largest-in-category round paired the estimating product with a national building-materials distributor, so the same auto-derived quantity that prices the job is also the quantity that gets ordered. Review these with *more* care, not less: a quantity error that used to surface as a thin margin now surfaces as the wrong material on the truck, a restocking fee, and a schedule hit. When a takeoff is bound to a supplier catalog, add two checks — (i) confirm the catalog SKU mapping matches the specified assembly (a "5/8\" Type X" line that maps to a standard board is a spec deviation, not a rounding error), and (ii) confirm the waste factor is applied *once* — in the takeoff or in the order, not both, and not neither.
- **Takeoffs produced by a self-assembled open-source pipeline.** A firm's own IT lead or a data-savvy PM can now stand up a free CAD/BIM-to-quantity pipeline (IFC / Revit / DWG exported to a spreadsheet, quantities extracted, then cost-matched against a public open-license work-item rate database of tens of thousands of items) using publicly available construction skill libraries and a general-purpose AI coding assistant. Nothing is wrong with this — but it is the **lowest-assurance** takeoff source in this list and must be reviewed accordingly, because it typically has: **no vendor QA**, **no confidence scores** to threshold against, **no audit trail** back to the sheet, and a **generic rate basis** not calibrated to the firm's local labor market, union agreements, or current material pricing. When the takeoff came from an in-house or open-source pipeline: treat *every* line as if it were sub-85% confidence (spot-check by dollar value per the 80/20 rule below), demand the extraction logic be stated (what the script counted and from which model property or layer — a quantity pulled from a BIM property set inherits every modeling error in the model), and explicitly flag the rate basis as generic-until-proven-local in the memo header.
...
5. **Takeoff source and method** — Fully manual / AI auto-count + human spot check / AI auto-count unchecked / AI + outsourced QA service / model-derived export (BIM, IFC, Revit) / in-house or open-source pipeline / bound to a supplier catalog. This changes which errors are most likely — and whether confidence scores exist to threshold against at all
...
4. Apply the **AI-Specific Failure-Mode Sweep** (when the takeoff is AI-generated or AI-assisted):
   ...
   - **Model-derived quantities (BIM / IFC / Revit extraction, incl. open-source pipelines):** a quantity read out of a model is only as good as the model. Flag when the takeoff came from a model export rather than the drawings: (i) model-vs-drawing divergence — the contract documents govern, and the model is frequently behind the latest bulletin; (ii) placeholder and generic families that carry no real assembly, which extract as a clean number and price as nothing; (iii) items modeled for coordination but not for quantity (a single lumped wall element, a duct run without fittings); (iv) anything the model simply does not contain (temp works, blocking, firestopping, accessories) — an extraction reports zero, and zero reads as "none required" rather than "not modeled." State in the memo that quantities were model-derived and name what the model does not carry.
   - **No-confidence-score sources:** when the pipeline reports no confidence at all (in-house scripts, open-source extraction, spreadsheet macros), the 85% threshold cannot be applied — fall back to the 80/20 dollar-value spot-check on every run and say so in the memo header rather than treating the absence of a flag as a clean result.
...
**Takeoff Source:** [AI tool + version / manual / hybrid / model-derived export / in-house or open-source pipeline]
**Quantity Basis:** [drawing-derived / model-derived (name the model + what it does not carry) | confidence scores: reported / none — 80/20 dollar spot-check applied instead | rate basis: firm actuals / vendor catalog / generic public rate DB — generic-until-proven-local | bound to a supplier catalog: yes (SKU mapping + waste-factor-applied-once checked) / no]
...
- Items under 85% confidence: [count — or "no confidence scores reported; 80/20 dollar spot-check applied"]
- Revision cloud sheets processed: [sheets]
- Hand-markup or redlined sheets: [sheets]
- Symbol-disambiguation items: [list]
- Model-derived quantities: [yes/no — if yes, name the model, its issue date vs. the current drawing revision, and what the model does not carry (temp works, blocking, firestopping, accessories)]
- Supplier-catalog binding: [none — or: SKU-to-spec mapping checked, waste factor confirmed applied once]
